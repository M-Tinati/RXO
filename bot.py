...
# متغیرهای سراسری
players = {}  # نگهداری اطلاعات بازی
turns = {}  # نوبت بازیکنان
game_messages = {}  # نگهداری پیام بازی برای بروزرسانی

# شروع بازی
@bot.message_handler(commands=['game'])
def start_game(message):
    chat_id = message.chat.id
    # در صورتی که بازی قبلاً شروع شده باشد
    if chat_id in players and players[chat_id]["player1"] is not None and players[chat_id]["player2"] is not None:
        bot.send_message(chat_id, "یک بازی در حال حاضر در جریان است.")
        return

    players[chat_id] = {"player1": "Player 1", "player2": "Player 2", "board": create_board()}
    turns[chat_id] = 1  # بازیکن 1 شروع می‌کند

    # ایجاد دکمه‌ها
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton(text="Show Board", callback_data="show_board"))
    markup.add(types.InlineKeyboardButton(text="Start Game", callback_data="start_game"))

    # نمایش پیام شروع بازی
    bot.send_message(chat_id, "بازی Connect Four شروع شد! روی دکمه‌ها کلیک کنید.", reply_markup=markup)

# نمایش بورد بازی
def display_board(chat_id):
    board = players[chat_id]["board"]
    board_str = print_board(board)
    
    # بروزرسانی پیام قبلی
    markup = types.InlineKeyboardMarkup()
    for i in range(7):
        markup.add(types.InlineKeyboardButton(text=str(i + 1), callback_data=f"move_{i}"))

    if chat_id in game_messages:
        bot.edit_message_text(f"نوبت {players[chat_id]['player1' if turns[chat_id] == 1 else 'player2']} است:\n\n{board_str}\n\nلطفاً یک ستون را انتخاب کنید:",
                              chat_id=chat_id,
                              message_id=game_messages[chat_id].message_id,
                              reply_markup=markup)
    else:
        game_message = bot.send_message(chat_id, f"نوبت {players[chat_id]['player1' if turns[chat_id] == 1 else 'player2']} است:\n\n{board_str}\n\nلطفاً یک ستون را انتخاب کنید:",
                                        reply_markup=markup)
        game_messages[chat_id] = game_message

# حرکت در بازی با استفاده از دکمه‌ها
@bot.callback_query_handler(func=lambda call: call.data.startswith('move_'))
def make_move(call):
    chat_id = call.message.chat.id
    column = int(call.data.split('_')[1])

    if chat_id not in players:
        bot.send_message(chat_id, "برای شروع بازی ابتدا دستور /game را وارد کنید.")
        return
    
    board = players[chat_id]["board"]
    player = turns[chat_id]
    row = -1
    ...
    if row == -1:
        bot.answer_callback_query(call.id, "این ستون پر است. لطفاً یک ستون خالی انتخاب کنید.")
        return
    ...
    if winner:
        bot.send_message(chat_id, f"بازیکن {'🔴 ' if player == 1 else '🔵 '} برنده شد!\n\n{print_board(board)}")
        players[chat_id] = {"player1": "Player 1", "player2": "Player 2", "board": create_board()}
        turns[chat_id] = 1
    else:
        turns[chat_id] = 2 if player == 1 else 1
        display_board(chat_id)

# بررسی برنده
def check_winner(board, row, col):
    player = board[row][col]
    directions = [(1, 0), (0, 1), (1, 1), (1, -1)]  # راست، پایین، قطر بالا به پایین، قطر پایین به بالا
...
# نمایش بورد
@bot.callback_query_handler(func=lambda call: call.data == "show_board")
def show_board(call):
    chat_id = call.message.chat.id
    display_board(chat_id)
